        ...
        <div className="flex items-center gap-4 md:gap-6">
          ...
          >
            <div className="w-12 h-12 rounded-full bg-white flex items-center justify-center">
              <img src="/icons/icon-1.svg" alt="Speciality" className="w-7 h-7" />
            </div>
            <div className="flex flex-col">
              <span className="text-white/70 text-md">Speciality</span>
              <div className="flex items-center gap-1">
                <span className="text-white font-semibold text-sm md:text-lg xl:text-xl whitespace-nowrap text-ellipsis max-w-[150px] overflow-hidden">{specialty}</span>
                <ChevronDown className={`w-4 h-4 text-white transition-transform ${isSpecOpen ? "rotate-180" : ""}`} />
              </div>
            </div>
            ...
            {isSpecOpen && (
              <div className="absolute top-full left-0 mt-2 w-56 bg-white rounded-xl shadow-lg py-2 z-50 overflow-hidden">
                {MOCK_SPECIALTIES.map((item) => (
                  <div 
                    key={item}
                    className={`px-4 py-2 hover:bg-gray-100 cursor-pointer text-[#12323A] ${specialty === item ? "font-bold bg-gray-50" : ""}`}
                    onClick={(e) => {
                      e.stopPropagation()
                      setSpecialty(item)
                      setIsSpecOpen(false)
          ...
          >
            <div className="w-12 h-12 rounded-full bg-white flex items-center justify-center">
              <img src="/icons/icon-2.svg" alt="Doctor" className="w-7 h-7" />
            </div>
            <div className="flex flex-col">
              <span className="text-white/70 text-md">Doctor</span>
              <div className="flex items-center gap-1">
                <span className="text-white font-semibold text-sm md:text-lg xl:text-xl whitespace-nowrap text-ellipsis max-w-[150px] overflow-hidden">{doctor}</span>
                <ChevronDown className={`w-4 h-4 text-white transition-transform ${isDocOpen ? "rotate-180" : ""}`} />
              </div>
            </div>
             ...
             {isDocOpen && (
              <div className="absolute top-full left-0 mt-2 w-56 bg-white rounded-xl shadow-lg py-2 z-50 overflow-hidden">
                {MOCK_DOCTORS.map((item) => (
                  <div 
                    key={item}
                    className={`px-4 py-2 hover:bg-gray-100 cursor-pointer text-[#12323A] ${doctor === item ? "font-bold bg-gray-50" : ""}`}
                    onClick={(e) => {
                      e.stopPropagation()
                      setDoctor(item)
                      setIsDocOpen(false)
